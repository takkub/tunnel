...
### 5. (Optional) ตั้งค่า API สำหรับลบ CNAME อัตโนมัติ

```bash
# 1. Copy .env.example
copy .env.example .env

# 2. แก้ไข .env และใส่ค่าจริง:
# CLOUDFLARE_API_TOKEN=your-token    # สร้างที่ https://dash.cloudflare.com/profile/api-tokens
# ZONE_ID=your-zone-id               # หาได้จาก Cloudflare Dashboard → เลือก domain
```

**ประโยชน์:**
- ✅ `npm run cleanup` จะลบ CNAME records อัตโนมัติ
- ✅ ไม่ต้องลบ CNAME ด้วยตนเองที่ Dashboard

**ไม่มี .env?**
- Script จะข้ามการลบ CNAME อัตโนมัติ
- แสดงวิธีลบ CNAME ด้วยตนเองแทน
...
| `npm run cleanup` | 🗑️ ลบ tunnel (แสดงรายการให้เลือก) |
| `npm run check-cnames` | 📋 เช็ค CNAME records ที่เหลืออยู่ |
| `npm run status` | ดูสถานะทั้งหมด |

🗑️ **Cleanup:** `npm run cleanup` - แสดงรายการ tunnel → เลือก → ยืนยัน → ลบทุกอย่าง!  
🔑 **ตั้งค่า API:** Copy `.env.example` → `.env` แล้วใส่ API Token และ Zone ID  
📝 **ลบ CNAME อัตโนมัติ:** ถ้ามี .env จะลบ CNAME ผ่าน API / ไม่มีก็แสดงวิธีลบเอง  
📋 **เช็ค CNAME:** `npm run check-cnames` - ดู CNAME ที่ยังค้างอยู่
