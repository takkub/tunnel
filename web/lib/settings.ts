...
// Raw values for server-side use only (building an S3 client, calling the
// Cloudflare GraphQL analytics API) — never return this from an API route,
// unlike getR2Settings()'s masked view above.
...
export function getR2Credentials(): R2Credentials {
  return {
    accountId: getR2Field('accountId', 'R2_ACCOUNT_ID'),
    accessKeyId: getR2Field('accessKeyId', 'R2_ACCESS_KEY_ID'),
    secretAccessKey: getR2Field('secretAccessKey', 'R2_SECRET_ACCESS_KEY'),
    bucket: getR2Field('bucket', 'R2_BUCKET'),
    analyticsToken: getR2Field('analyticsToken', null),
  }
}
