...
function renderMenu(tunnels, selectedIndex, mode) {
    ...
    console.log(`${ui.c.cyan}${ui.c.bold}║${ui.c.reset}  ${ui.c.brightCyan}Start Tunnel [${mode}]${ui.c.reset}${' '.repeat(Math.max(0, boxWidth - 22 - mode.length))}${ui.c.cyan}${ui.c.bold}║${ui.c.reset}`);
    console.log(`${ui.c.cyan}${ui.c.bold}╚${'═'.repeat(boxWidth - 2)}╝${ui.c.reset}`);
    ...
    tunnels.forEach((tunnel, index) => {
        ...
        if (isSelected) {
            console.log(`  ${prefix} ${ui.c.cyan}${ui.c.inverse} ${tunnel.displayName.padEnd(boxWidth - 6)} ${ui.c.reset}`);
        } else {
...
async function main() {
    try {
        ...
        ui.header('Start Tunnel', `Mode: ${mode}`);
        ...
        console.clear();
        ui.header('Starting Tunnel', selected.displayName);

        try {
            if (mode === 'native') {
                const pid = nativeStart(selected.name);
                console.log('');
                ui.success(`Tunnel ${selected.displayName} started (pid ${pid})`);
            } else {
                dockerStart(selected.name);
                console.log('');
                ui.success(`Tunnel ${selected.displayName} started`);
            }
            process.exit(0);
        } catch (error) {
            console.log('');
            ui.fail(`Failed to start ${selected.displayName}: ${error.message}`);
            process.exit(1);
        }
